...
一升に含まれるお米の数について、64827(虫や鮒)という覚え方もあります。
...
int main(int argc, char const *argv[]) {
  ...
  // 2^ 1 = 2 から始まり、
  // 2^62 = 4611686018427387904 までは正しく出力されています。
  // 2^63 = -9223372036854775808 と負号が付いています。
  // 2^64 からは 0 と出力されます。
  // long型は 64bit, -2^63 〜 +2^63-1 までの範囲の整数を表すことができます。
  // この範囲を超えたため、出力結果が不正となりました。

  // そこで、整数型による演算を手放し、浮動小数点数による演算を行うこととします。
  // double型は 2.225074 10^-308 < double の絶対値 < 1.797693 10^308 までの
  // 範囲の数を表現できます。
  for (n = 1; n <= 81; n++) {
    printf("%2d %28.2f\n", n, pow((double)2, (double)n));
  }
  // 二の冪乗を求めているため、とても正確に計算できています。
  // 2^81 から 1を引いて 賜るお米の数を求めてみましょう。
  for (n = 1; n <= 81; n++) {
    printf("%2d %28.2f\n", n, pow((double)2, (double)n) -1);
  }
  // 2のn乗は偶数です。そこから1を引いたので奇数となるはずですが、
  // 途中からは偶数の値が出力されています。
  // double型の有効桁数は 二進数で53桁、十進数で約15桁です。

  // 賜るお米の粒の数 (近似値ですが、十分な精度です)
  double grains = pow((double)2, (double)81) - 1;
  // 升
  double shou   = grains / 64827;
  // 斗
  double to     = shou / 10;
  // 俵
  double hyou   = to / 4;
  // 石
  double koku   = to / 10;
  printf ("賜るお米の粒の数: %25.0f\n", grains);
  printf ("　　　　　　　升: %25.0f\n", shou);
  printf ("　　　　　　　斗: %25.0f\n", to);
  printf ("　　　　　　　俵: %25.0f\n", hyou);
  printf ("　　　　　　　石: %25.0f\n", koku);
  // 37京2969兆8488億6378万4832石 と求めることが出来ました。

  // 蔵の体積を求めます。蔵に米俵を隙間なく詰めると何俵入るでしょうか。
  double kura    = 7.2 * 4.5 * 5.4; // ㎥（立方メートル）
  double tawara  = 180.0 * // 一合の体積は180ml
                    10   * // 一升
                    10   * // 一斗
                     4   / // 一俵
                  1000   / // L（リットル）単位に変換
                  1000;    // ㎥（立方メートル）単位に変換
  double irisuu  = kura / tawara;
  double kurasuu = hyou / irisuu;
  printf ("蔵の体積　　　　　　: %25.3f\n", kura);
  printf ("俵の体積　　　　　　: %25.3f\n", tawara);
  printf ("蔵に入る俵の数　　　: %25.3f\n", irisuu); // 一蔵2430俵入り
  printf ("必要な蔵の数　　　　: %25.3f\n", kurasuu);
  ...
  double fujisan        = 18500 * 18500 * 3.14 * 3776 / 3;
  double irisuu_fujisan = fujisan / tawara;
  double fujisansuu     = hyou / irisuu_fujisan;
  printf ("富士山の体積　　　　: %25.3f\n", fujisan);
  printf ("俵の体積　　　　　　: %25.3f\n", tawara);
  ...
  double bairitsu       = pow(fujisansuu, 1.0 / 3.0);    // 立方根
  ...
  // 地球の赤道から北極までの長さは一万粁(キロメートル)です。
  // ですので、地球の円周は40000km、直径12740km、半径6370km になります。
  double menseki_chikyu  = 4 * 3.14 * pow((double)6370, (double)2);
  double menseki_fujisan = 3.14 * pow((double)18.5, (double)2);
  printf ("地球の表面積　　　　: %25.3f\n", menseki_chikyu);
  ...
  double tan             = koku   /  4;     // 田んぼの広さ 単位：反
  double choubu          = tan    / 10;     // 田んぼの広さ 単位：町歩 = 1ha
  double km2             = choubu / 100;    // 田んぼの広さ 単位：㎢
  double nippon          = km2    / 378000; // 田んぼの広さ 単位：日本
  double chikyu          = km2    / 509645864; // 田んぼの広さ 単位：地球
  printf ("反　　　　　　　　　: %21.0f\n", tan);
  printf ("町歩　　　　　　　　: %21.0f\n", choubu);
  printf ("平方キロメートル　　: %21.0f\n", km2);
  printf ("日本　　　　　　　　: %21.0f\n", nippon);
  printf ("地球　　　　　　　　: %21.0f\n", chikyu);
  printf ("太陽　　　　　　　　: %21.0f\n", chikyu/109/109);

  // 37京2969兆8488億6378万4832石は、
